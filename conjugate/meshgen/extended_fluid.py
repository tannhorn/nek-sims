...
def add_physical_groups(
    ...
    top_group = gmsh.model.addPhysicalGroup(1, [top_lines[0]], tag=4)
    gmsh.model.setPhysicalName(1, top_group, "TopWall")

    top_group = gmsh.model.addPhysicalGroup(1, [top_lines[1]], tag=5)
    gmsh.model.setPhysicalName(1, top_group, "TopFree")

    # Inner surfaces
    # add_special_boundary_group(
    #     lines, exclude, [left_lines, right_lines, bottom_lines, top_lines], tag=6
    # )

    # Physical group for all surfaces
    surface_group = gmsh.model.addPhysicalGroup(2, surfaces, tag=101)
...
def generate_data(laminar: bool, Re: float, nelq: int = 7, yplus: float = 1.0):
    ...
    # symmetric x/y
    coord_x = [0.0, xlen_wall, xlen_wall + xlen_outlet]
    coord_y = [ymin, ymax]
    ...
    # discretization
    disc_x = [num_x_wall, num_x_outlet]
    disc_y = [num_y_prog]

    # element size progression
    prog_x = [1, 1]
    prog_y = [1 / growth_rate]
...
if __name__ == "__main__":
    ...
    mesh_and_save(surfaces, "extended_fluid.msh")
